...
program
  ...
  .option('-u, --username [username]', "当前操作的用户", "shushanfx")
  ...
  if (program.action) {
    switch (program.action) {
      case 'copy':
        ...
        let result = await Index.copyProject(program.id, program.to, program.username || 'shushanfx');
        switch (result.code) {
          case 1:
      ...
      case 'set':
        ...
        let buildList = (a) => {
          if (a && typeof a === 'string') {
            return a.split(',').filter(item => item);
          }
          return [];
        }
        let containAll = (a, b) => {
          ...
          let copyOfB = b.map(item => item);
          a.forEach(item => {
            let index = copyOfB.indexOf(item);
            if (index >= 0) {
              copyOfB.splice(index, 1);
            }
          })
          return copyOfB.length === 0;
        }
        let tags = buildList(tag);
        ...
        if (list && list.length && port && host) {
          ...
          list = list.filter(item => {
            let curTags = buildList(item.tag);
            if (item.isProxy && containAll(curTags, tags)) {
              return true;
            }
            return false;
          })
